...
Δικαίωμα προβολής έχει αποκλειστικά χρήστης που έχει κάνει login πρώτα και έχει προστέσει το uuid του ως header στο "authorization". Διαφορετικά, το /showincart δεν θα λειτουργήσει.

## /removeincart

Δίνοντας 0.0.0.0:5000/removeincart με μέθοδο DELETE, μπορεί ένας user να αφαιρέσει προϊόντα από το καλάθι. Το id του προϊόντος και η ποσότητα θα δοθεί ως ένα json αρχείο της μορφής:

```json
{"_id" : "something" 
  }
 ```
Δικαίωμα διαγραφής έχει αποκλειστικά χρήστης που έχει κάνει login πρώτα και έχει προστέσει το uuid του ως header στο "authorization". Διαφορετικά, το /removeincart δεν θα λειτουργήσει.

## /buy

Δίνοντας 0.0.0.0:5000/buy με μέθοδο POST, μπορεί ένας user να αγοράσει τα προϊόντα οπού έχει στο καλάθι του. Το PIN της κάρτας θα δοθεί ως ένα json αρχείο της μορφής:

```json
{"card" : "16PIN" 
  }
 ```
Δικαίωμα προσθεσης έχει αποκλειστικά χρήστης που έχει κάνει login πρώτα και έχει προστέσει το uuid του ως header στο "authorization". Διαφορετικά, το /buy δεν θα λειτουργήσει.

## /history

Δίνοντας 0.0.0.0:5000/history με μέθοδο GET, μπορεί ένας user να δει το ιστορικό των παραγγελιών του. 

Δικαίωμα προσθεσης έχει αποκλειστικά χρήστης που έχει κάνει login πρώτα και έχει προστέσει το uuid του ως header στο "authorization". Διαφορετικά, το /history δεν θα λειτουργήσει.
...
Δίνοντας 0.0.0.0:5000/addProducts με μέθοδο POST, μπορεί ένας admin να προσθέσει προϊόντα στην βάση δεδομένων. Τα στοιχεία του προϊόντος θα δοθούν ως ένα json αρχείο της μορφής:
...
Δίνοντας 0.0.0.0:5000/removeid με μέθοδο DELETE, μπορεί ένας admin να διαγράψει προϊόντα από την βάση δεδομένων. Τo id του προϊόντος προς διαγραφή θα δοθεί ως ένα json αρχείο της μορφής:
...
Δίνοντας 0.0.0.0:5000/patchproducts με μέθοδο PATCH, μπορεί ένας admin να τροποποιήσει πληροφορίες προϊόντων από την βάση δεδομένων με χρήση του id. Τo id και το οποιοδήποτε στοιχείο θέλουμε να τροποποιηθεί , θα  δοθούν ως ένα json αρχείο της μορφής:
